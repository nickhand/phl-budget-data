...
class SalesCollectionsBySector(ETLPipeline):
    ...
    def __post_init__(self) -> None:
        ...
        if not self.path.exists():
            raise FileNotFoundError(f"No PDF available for fiscal year '{self.fiscal_year}'")
    ...
    def extract(self) -> pd.DataFrame:
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            cropped = pg.crop((pg.bbox[0], top_left.top, pg.bbox[2], bottom_left.bottom + 3))

            # Table strategy based on format
            horizontal_strategy = "lines" if self.legacy else "text"
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        main_industries = data.query(
            "parent_sector.isnull() and sector != 'Subtotal'"
            " and sector != 'Motor Vehicle Sales Tax'"
        )
