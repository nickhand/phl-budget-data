...
class DepartmentObligationsSchema(BaseModel):
    ...
    variable: Literal[
        ...
        title="Variable",
        description="The variable type; one of 'Actual', 'Target Budget', 'Adopted Budget',"
        " 'Current Projection'",
    )
    ...
    dept_name: str = Field(title="Department Name", description="The name of the department.")
    as_of_date: datetime.date = Field(title="As of Date", description="The date of the value.")
...
def remove_line_items(data: pd.DataFrame, start: str, *stops: str) -> pd.DataFrame:
    ...
    remove = []
    while i < len(data) and not any(data.loc[i, "dept_name"].startswith(stop) for stop in stops):
        remove.append(i)
        i += 1
...
class DepartmentObligations(ETLPipelineQCMR):
    ...
    def extract(self) -> pd.DataFrame:
        ...
        for pg_num in range(1, self.num_pages + 1):
            ...
            df = self._get_textract_output(pg_num=pg_num, concat_axis=1, remove_headers=True)
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if ncol != expected_ncol:
            raise ValueError(f"Unexpected number of columns: got {ncol}, expected {expected_ncol}")

        # Assign columns
        column_names = get_column_names(self.fiscal_year, self.quarter)
        data = data.rename(columns=dict(zip(data.columns[1:], column_names, strict=False))).drop(
            columns=[""]
        )
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        data = data.loc[~data["dept_name"].str.startswith("Employee Benefits: ", na=False)]

        citywide = data.groupby(["variable", "fiscal_year", "time_period"])["total"].sum()
