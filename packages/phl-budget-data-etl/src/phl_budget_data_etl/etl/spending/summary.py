...
@dataclass
class BudgetSummaryBase(ETLPipelineAWS):
    """
    ...
    def extract(self) -> pd.DataFrame:
        ...
        for start, stop in zip(starts, stops, strict=False):
            ...
            df = (
                ...
                .iloc[1:]
                .rename(
                    columns=dict(zip(old_columns, new_columns, strict=False)),
                )
            )
        ...
        out["dept_name"] = out["dept_name"].str.replace(r"\(\d\)", "", regex=True).str.strip()

        return pd.concat(
            [
                out,
                pd.DataFrame(
                    [["Total", total.iloc[1], total[data.columns[-1]], "General Fund"]],
                    columns=out.columns,
                ),
            ],
            axis=0,
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        if self.fiscal_year <= 2015:
            ...
            totals = pd.read_excel(path, sheet_name=1).rename(columns={"Category": "major_class"})
    ...
    def load(self, data: pd.DataFrame) -> None:
        ...
        # Get the path
        tag = str(self.fiscal_year)[2:]
        path = self.get_data_directory("processed") / self.kind / self.flavor / f"FY{tag}.csv"
    ...
    def extract_transform_load_all(cls, fresh: bool = False) -> None:
        ...
        for pdf_path in cls.get_pdf_files():
            ...
            # Get the output path
            output_path = Path(str(pdf_path).replace("raw", "processed").replace(".pdf", ".csv"))
