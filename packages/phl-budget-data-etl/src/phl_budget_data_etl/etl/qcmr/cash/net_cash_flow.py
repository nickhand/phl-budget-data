...
class CashReportNetCashFlow(CashFlowForecast):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) != len(CATEGORIES):
            ...
            raise ValueError(f"Parsing error for net cash flow data in {tag} cash report")
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        for total_column, _cats_to_sum in groups.items():
            X = data.query("category in  @cats_to_sum").groupby("fiscal_month")["amount"].sum()
            Y = data.query(f"category == '{total_column}'").set_index("fiscal_month")["amount"]
            diff = (X - Y).abs()
