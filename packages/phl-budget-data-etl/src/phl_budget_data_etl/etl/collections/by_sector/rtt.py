...
class RTTCollectionsBySector(ETLPipeline):
    ...
    def __post_init__(self) -> None:
        ...
        if self.month is not None:
            ...
            self.path = self.get_data_directory("raw") / f"{self.year}_Q{self.quarter}.pdf"
            if not self.path.exists():
                raise FileNotFoundError(
    ...
    def extract(self) -> pd.DataFrame:
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            all_words = extract_words(pg, keep_blank_chars=True, x_tolerance=1, y_tolerance=1)

            ## TOP LEFT
            top_left = min(
                [w for w in all_words if w.text.strip().lower().startswith("non-residential")],
                key=lambda w: w.x0,
            )
            ...
            # Group into rows
            d = []
            for _k, v in fuzzy_groupby(words, lower_tol=1, upper_tol=1).items():
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data = data[[0, 1, 2]] if self.year > 2019 else data[[0, 7, 8]]
