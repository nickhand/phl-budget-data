...
class WageCollectionsBySector(ETLPipeline):
    ...
    def __post_init__(self) -> None:
        ...
        assert self.num_pages in [1, 2, 3, 4, 5]
    ...
    def extract(self) -> pd.DataFrame:
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            pg = pdf.pages[-1] if self.quarterly else pdf.pages[0]

            # Determine crop areas
            all_words = extract_words(pg, keep_blank_chars=True, x_tolerance=2, y_tolerance=1)

            ## TOP LEFT
            top_left = min(
                [w for w in all_words if w.text.strip().lower().startswith("construction")],
                key=lambda w: w.x0,
            )

            ## BOTTOM LEFT
            bottom_left = min(
                [w for w in all_words if w.text.strip().lower().startswith("unclassified")],
                key=lambda w: w.x0,
            )

            # Crop the main part of the document and extract the words
            cropped = pg.crop((top_left.x0, top_left.top, pg.bbox[2], bottom_left.bottom))
            words = extract_words(cropped, keep_blank_chars=True, x_tolerance=2, y_tolerance=1)

            # Group into rows
            d = []
            for _k, v in fuzzy_groupby(words, key="bottom", lower_tol=3, upper_tol=3).items():
