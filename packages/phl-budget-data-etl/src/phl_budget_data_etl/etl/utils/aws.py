...
def remove_nonnumeric_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(0, len(df.index)):
        ...
        test = row.str.match(r"([A-Za-z\(\)]|^$|20\d{2}|\d{2})").all()
        if not test:
            break
...
def parse_pdf_with_textract(
    ...
    # Initialize the PDF
    with pdfplumber.open(pdf_path) as pdf, tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pg_num, pg in enumerate(pdf.pages, start=1):
            ...
            r = TextractResponse.model_validate(
                textract.analyze_document(
                    Document={"S3Object": {"Bucket": bucket_name, "Name": filename.name}},
                    FeatureTypes=["TABLES"],
                )
            )
