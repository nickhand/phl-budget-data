...
class CashReportParams(BaseModel):
    kind: CASH_DATA_TYPE
...
def _load_department_reports(cls: type[ETLPipelineQCMR]) -> pd.DataFrame:
    ...
    for f, fiscal_year, quarter in _load_processed_results(cls):
        ...
        historical_actuals = df.query("variable == 'Actual' and time_period == 'Full Year'")
        ...
        adopted_budget = df.query("variable == 'Adopted Budget' and time_period == 'Full Year'")
    ...
    return out.sort_values(["report_fiscal_year", "report_quarter"], ascending=False).reset_index(
        drop=True
    )
...
def load_fulltime_positions() -> pd.DataFrame:
    ...
    duplicates = actuals.loc[actuals.duplicated(subset=["as_of_date", "fund", "dept_code"])]
    df = df.drop(duplicates.index)
...
load_cash_reports.model = CashReportParams  # type: ignore
