...
def decimal_to_comma(df: pd.DataFrame, usecols: list[str] | None = None) -> pd.DataFrame:
...
def fix_zeros(df: pd.DataFrame, usecols: list[str] | None = None) -> pd.DataFrame:
...
def replace_commas(df: pd.DataFrame, usecols: list[str] | None = None) -> pd.DataFrame:
...
def convert_to_floats(
    ...
    for col in usecols:
        df[col] = pd.to_numeric(
            df[col].replace(r"[\$,)]", "", regex=True).replace(r"[(]", "-", regex=True),
            errors=errors,
        )
    return df
...
def remove_missing_rows(df: pd.DataFrame, usecols: list[str] | None = None) -> pd.DataFrame:
...
def fix_duplicated_chars(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for index in duplicated:
        data.loc[index] = data.loc[index].str.replace(r"([a-zA-Z0-9%,.])\1{1,2}", r"\1", regex=True)
...
def remove_empty_columns(data: pd.DataFrame, use_nan: bool = False) -> pd.DataFrame:
    ...
    # Remove null columns
    bad_columns = data.isnull().all() if use_nan else (data == "").all()
