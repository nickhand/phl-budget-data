"""Load processed collections data."""
...
def load_sales_collections_by_sector() -> pd.DataFrame:
    ...
    return out.sort_values(["fiscal_year", "parent_sector", "sector"], ascending=True).reset_index(
        drop=True
    )
...
def load_birt_collections_by_sector() -> pd.DataFrame:
    ...
    return out.sort_values(["tax_year", "parent_sector", "sector"], ascending=True).reset_index(
        drop=True
    )
...
def load_rtt_collections_by_sector() -> pd.DataFrame:
    ...
    for f in files:
        # Quarterly data
        if "Q" in f.stem:
            year_str, quarter_str = f.stem.split("-")
            year = int(year_str)
            quarter = int(quarter_str[-1])

            # Get the fiscal year
            fiscal_year = year if quarter in [1, 2] else year + 1
        ...
        # Melt the data
        X = X.melt(
            id_vars=["category", "parent_category"],
        ).assign(
    ...
    out["month_start"] = out["fiscal_quarter"].replace({1: "07", 2: "10", 3: "01", 4: "04"})

    # Add date
    out["date"] = pd.to_datetime(out.apply(lambda r: f"{r['year']}-{r['month_start']}", axis=1))

    return out.sort_values("date", ascending=False, ignore_index=True).drop(columns=["month_start"])
...
def _load_monthly_collections(files: Iterable[Path], total_only: bool = False) -> pd.DataFrame:
    ...
    for f in sorted(files, reverse=True):
        # Get month/year
        year_str, month_str, *_ = f.stem.split("-")
        year = int(year_str)
        month = int(month_str)
        month_name = calendar.month_abbr[month].lower()
        ...
        if keep_kind:
            cols.append("kind")
        X = df[cols].rename(columns=dict(zip([a, b], [fiscal_year, fiscal_year - 1], strict=False)))
        ...
        X["total"] = pd.to_numeric(X["total"], errors="coerce").fillna(0)
    ...
    out["date"] = pd.to_datetime(
        out["month"].astype(str) + "/" + out["year"].astype(str), format="%m/%Y"
    )
...
def load_city_collections() -> pd.DataFrame:
    ...
    out = []
    for cls, label, tag in zip(classes, labels, tags, strict=False):
        # Get the path to the files to load
        dirname = cls.get_data_directory("processed")
        files = dirname.glob(f"*-{tag}.csv")

        # Load
        out.append(_load_monthly_collections(files, total_only=(tag == "tax")).assign(kind=label))
