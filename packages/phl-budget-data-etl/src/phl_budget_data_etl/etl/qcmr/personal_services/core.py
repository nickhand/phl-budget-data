...
class PersonalServicesSchema(BaseModel):
    ...
    variable: Literal[
        ...
        title="Variable",
        description=(
            "The spending variable type; one of 'Actual', 'Target Budget',"
...
def _to_tidy_format(X: pd.DataFrame, fy: int, qtr: int) -> pd.DataFrame:
    ...
    if len(X) != 4:
        if dept.startswith("TOTAL GENERAL FUND"):
            ...
            raise ValueError(f"Data for department '{dept}' has length {len(X)}, expected 4.")
    ...
    if len(X) == 4:
        ...
        if not test.all():
            print(X[X.columns[0]])
            raise ValueError(f"Data for department '{dept}' has non-empty values in the first row.")
...
class PersonalServices(ETLPipelineQCMR):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for footnote in footnotes:
            data["0"] = (
                data["0"].str.replace(footnote, "", regex=False).str.strip().replace("", np.nan)
            )
