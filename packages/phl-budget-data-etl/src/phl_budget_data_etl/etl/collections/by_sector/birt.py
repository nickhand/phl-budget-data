...
class BIRTCollectionsBySector(ETLPipeline):
    ...
    def extract(self) -> pd.DataFrame:
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            for pg in pdf.pages:
                ...
                tax_year = df[1].str.extract(r"Tax Year (\d{4})").dropna().squeeze()
                X["tax_year"] = tax_year
                out.append(X)
                ...
                if len(df.columns) > 5:
                    ...
                    tax_year = df[5].str.extract(r"Tax Year (\d{4})").dropna().squeeze()
                    X["tax_year"] = tax_year
                    out.append(X)
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        data = (
            data.pipe(tr.remove_spaces).pipe(tr.convert_to_floats, usecols=data.columns[1:])
