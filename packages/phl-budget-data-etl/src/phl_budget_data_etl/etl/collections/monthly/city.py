...
def find_top_cutoff(pg: pdfplumber.page.Page) -> float:
    ...
    if has_header:
        ...
        top = rects[1]["bottom"] if "Comparative Statement" in pg_text else rects[0]["bottom"]

    return top


def find_footer_cutoff(pg: pdfplumber.page.Page) -> dict[str, float] | None:
...
class CityCollectionsReport(MonthlyCollectionsReport):
    ...
    def extract(self) -> pd.DataFrame:
        ...
        with pdfplumber.open(self.path) as pdf:
            ...
            for _pg_num, pg in enumerate(pdf.pages, start=1):
                ...
                bottom = footer_cutoff["bottom"] if footer_cutoff is not None else pg.height

                # Crop the main part of the document and extract the words
                cropped = pg.crop((0, top, pg.width, bottom))
                words = extract_words(cropped, keep_blank_chars=False, x_tolerance=1, y_tolerance=1)
