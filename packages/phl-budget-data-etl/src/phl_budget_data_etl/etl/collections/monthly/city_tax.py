...
class CityTaxCollections(CityCollectionsReport):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        stop = data.index[data[0].str.contains("TOTAL TAX REVENUE")]
        assert len(stop) == 1
        stop = stop[0]
        ...
        tax = tax.pipe(remove_missing_rows, usecols=tax.columns[1:]).reset_index(drop=True)
        ...
        for i, suffix in enumerate(["", "pica", "city"]):
            suffix = f"{suffix}_total" if suffix else "total"
